...
pub fn add_after_head(head: &Rc<RefCell<CacheList>>, node: &Rc<RefCell<CacheList>>) {
    ...
    if let Some(old_first) = head.next.take() {
        mut_node.prev = Some(Rc::downgrade(&old_first));
        drop(mut_node);
        ...
        old_first.borrow_mut().next = Some(node.to_owned());
        drop(old_first);
    } else {
        head.next = Some(node.clone());
    }
...
pub fn move_node_to_head(
    head: &Rc<RefCell<CacheList>>,
    node: &Weak<RefCell<CacheList>>,
    data: Option<&Vec<u8>>
) {
    if let Some(current_node) = node.upgrade() {
        let mut current_mut_node = current_node.borrow_mut(); //got the current node
        ...
        if let Some(prev_weak) = &current_mut_node.prev {
            ...
            if let Some(prev_rc) = prev_weak.upgrade() {
                let mut prev_node = prev_rc.borrow_mut();
        ...
        add_after_head(head, &current_node)
    }
}

//TODO : implement last node purge
pub fn purge(cache: &mut Cache) {}
